...
public class zPHomes extends JavaPlugin {
    ...
    public void onEnable(){ // Put that in config file
        ...
        if(!configdata.exists()){
            ...
            //saveDefaultConfig();
            saveConfig();
            cs.sendMessage("Configuration File created, setup your Mysql/Mariadb details there");
            cs.sendMessage("Disabling zPHomes Plugin, setup your MySQL/MariaDB Database connection in ./plugins/zPHomes/config.yml");

            this.getPluginLoader().disablePlugin(Bukkit.getPluginManager().getPlugin("solarsystem.coffee.zPHomes"));
            ...
            try {
                ...
                if(Version<4){
                    stmt.execute("ALTER TABLE homes ADD FLOAT() yaw DEFAULT='-1.0', FLOAT() pitch DEFAULT='-1.0', varchar(255) server DEFAULT='DEFAULT'");
    ...
    public boolean onCommand(CommandSender interpreter, Command cmd, String input, String[] args){
        ...
        try {
            conn = DriverManager.getConnection("jdbc:mysql://"+Address+"/"+Database,DatabaseUser,Password);
            stmt = (Statement) conn.createStatement();
            stmt.execute("CREATE TABLE IF NOT EXISTS homes (ID int PRIMARY KEY NOT NULL AUTO_INCREMENT, UUID varchar(255), Name varchar(255), world varchar(255), x double, y double, z double, yaw float, pitch float)");
        ...
        if(interpreter instanceof Player){
            ...
            if(input.equals("sethome")){ //SET new Home
                ...
                try {
                    ...
                    if(stmt.execute("INSERT IGNORE INTO homes (UUID,Name,world,x,y,z,yaw,pitch,server) VALUES ('"
                            +playeruuid+"', '"+home+"', '"+player.getWorld().getName()+"', "
                            +loc.getX()+", "+loc.getY()+", "+loc.getZ()+", "
                            +loc.getYaw()+", " +loc.getPitch()+", '"+player.getServer().getName()+")"))
